...
| **ACC-CURSOR Declaration** | DB2 Cursor | INQACC.cbl | Repository SELECT Query | `com.inqacc.repository.AccountRepository.findBySortcodeAndNumber()` | Execute parameterized SQL SELECT with WHERE clause: `ACCOUNT_SORTCODE = ? AND ACCOUNT_NUMBER = ?`; reserved `99999999` branch selects highest `ACCOUNT_NUMBER` for sortcode |
...
|---|---|---|---|---|---|---|
| **ACCDB2.cpy** | ACCOUNT table declaration (12 columns) | DB2 schema reference | `com.inqacc.entity.AccountEntity` | `src/main/java/com/inqacc/entity/` | 12 | POC response policy returns complete 12-field records in successful responses |
| **ACCOUNT.cpy** | ACCOUNT-DATA structure (11 fields, 1 redefine group) | COBOL working storage structure | `com.inqacc.dto.AccountResponseDto` | `src/main/java/com/inqacc/dto/` | 12 (API response fields) | Mirrors ACCDB2 12-column schema; field ordering preserved for legacy parity |
...
| ACCOUNT-CUST-NO | ACCOUNT_CUSTOMER_NUMBER | Numeric | 9(10) | CHAR(10) | String | `customerNumber` | 10-digit numeric; left-padded with zeros | No (POC response policy) |
...
| ACCOUNT-TYPE | ACCOUNT_TYPE | Character | X(8) | CHAR(8) | String | `accountType` | 8-char account classification (e.g., 'CHECKING', 'SAVINGS ') | No (POC response policy) |
| ACCOUNT-INTEREST-RATE | ACCOUNT_INTEREST_RATE | Numeric Packed | 9(4)V99 | DECIMAL(4,2) | BigDecimal | `interestRate` | 2 decimal places; range 0.00–99.99 | No (POC response policy) |
| ACCOUNT-OPENED | ACCOUNT_OPENED | Date | 9(8) | DATE | LocalDate | `accountOpened` | Legacy date representation converted to ISO `yyyy-MM-dd` | No (POC response policy) |
...
| ACCOUNT-OVERDRAFT-LIMIT | ACCOUNT_OVERDRAFT_LIMIT | Numeric | 9(8) | INTEGER | Long | `overdraftLimit` | Positive integer; currency in pence (GBP implied) | No (POC response policy) |
| ACCOUNT-LAST-STMT-DATE | ACCOUNT_LAST_STATEMENT | Date | 9(8) | DATE | LocalDate | `lastStatementDate` | Legacy date representation converted to ISO `yyyy-MM-dd` | No (POC response policy) |
...
| ACCOUNT-NEXT-STMT-DATE | ACCOUNT_NEXT_STATEMENT | Date | 9(8) | DATE | LocalDate | `nextStatementDate` | Legacy date representation converted to ISO `yyyy-MM-dd` | No (POC response policy) |
...
| ACCOUNT-AVAILABLE-BALANCE | ACCOUNT_AVAILABLE_BALANCE | Numeric Packed | S9(10)V99 | DECIMAL(10,2) | BigDecimal | `availableBalance` | 2 decimal places; signed; currency in pence (GBP) | No (POC response policy) |
| ACCOUNT-ACTUAL-BALANCE | ACCOUNT_ACTUAL_BALANCE | Numeric Packed | S9(10)V99 | DECIMAL(10,2) | BigDecimal | `actualBalance` | 2 decimal places; signed; currency in pence (GBP) | No (POC response policy) |

**Notes:**
- **Composite Key:** ACCOUNT-SORT-CODE + ACCOUNT-NUMBER form unique identifier; required in HTTP path parameters
- **Date Transformation:** Source DB2 DATE values (carried through legacy transport representations) are converted to ISO 8601 string (JSON response).
...
- **POC Nullable Policy:** Successful responses contain all 12 fields (no omitted/null fields in normal POC records).
...
|---|---|---|---|---|
| **FR-001** | Execute account lookup by composite key (ACCOUNT_SORTCODE + ACCOUNT_NUMBER), with reserved `99999999` branch for highest-account-by-sortcode behavior | BR-001 | `AccountInquiryService.findAccountBySortcodeAndNumber()`, `AccountRepository.findBySortcodeAndNumber()`, `AccountInquiryController.getAccountBySortcodeAndNumber()` | TC-001, TC-002, TC-003, TC-004 |
...
| **FR-008** | Return standardized JSON error envelope with `error.code`, `error.message`, `error.correlationId`, and `error.timestamp` | BR-012 | `ErrorEnvelopeDto`, `@ExceptionHandler` methods (global), `CorrelationIdInterceptor` | TC-019, TC-020, TC-021 |
...
| **BR-007** | The system must return standardized JSON error envelope with `error.code`, `error.message`, `error.correlationId`, and `error.timestamp`. | Any error condition | TC-019 | Unit, Integration | JSON error envelope returned with canonical nested fields |
