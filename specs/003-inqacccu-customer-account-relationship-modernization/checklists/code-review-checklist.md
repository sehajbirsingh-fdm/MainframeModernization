...
| ARCH-B-003 | **REST Endpoint Mapping** - Endpoint paths conform to OpenAPI 3.0.3 contract | ISSUE FOUND - Endpoint path exists as `GET /api/v1/customers/{customerNumber}/accounts`, but runtime response model (`customer`, `accounts.count`) differs from frozen contract fields (`customerNumber`, `numberOfAccounts`, flat `accounts[]`) | CRITICAL | ISSUE FOUND |
...
| TEST-U-002 | **Service Layer Tests** - Business logic tested in isolation | PARTIAL - `AccountRelationshipServiceTest` covers found/not-found orchestration; validation branch is exercised primarily at controller/frontend layers | HIGH | PARTIAL |
